...
app.post('/api/posts', authenticateToken, checkRestriction, (req, res, next) => {
    ...
    // 🔧 Dinamik multer - 'media', 'images', veya 'image' field'larını kabul et
    const uploadMiddleware = upload.fields([
        { name: 'media', maxCount: UPLOAD_CONFIG.maxFilesPerUpload },
        { name: 'images', maxCount: UPLOAD_CONFIG.maxFilesPerUpload },
        { name: 'image', maxCount: UPLOAD_CONFIG.maxFilesPerUpload }
    ]);
    ...
    uploadMiddleware(req, res, (err) => {
        ...
        // Dosyaları tek bir diziye normalize et
        if (req.files) {
            const allFiles = [];
            if (req.files.media) allFiles.push(...req.files.media);
