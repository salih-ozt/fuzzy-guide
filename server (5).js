        ...
        socket.on('chatsee:message', async (data, ack) => {
            try {
                ...
                const payload = {
                    ...
                    is_deleted      : false,
                    isDeleted       : false,
                    created_at      : msgTimestamp,     // HTML msg.created_at bekliyor
...
function mobileKeyMiddleware(req, res, next) {
    // Content-Type'a ASLA dokunma — res.json() ve sendFile zaten doğru set eder.
    // Sadece /api/ istekleri için cache-control ayarla.
    if (req.path.startsWith('/api/')) {
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, private');
    }
    next();
...
app.get('*', (req, res, next) => {
    ...
    if (fss.existsSync(htmlPath)) {
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.setHeader('Cache-Control', 'no-cache');
        return res.sendFile(require('path').resolve(htmlPath));
    }
