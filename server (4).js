    ...
    io.on('connection', (socket) => {
        ...
        socket.on('chatsee:message', async (data, ack) => {
            try {
                ...
                const payload = {
                    // Her alan hem yeni hem eski isimle gönderiliyor (HTML uyumu)
                    id              : msgId,
                    conversation_id : conversationId,   // HTML msg.conversation_id bekliyor
                    conversationId,
                    sender_id       : userId,           // HTML msg.sender_id bekliyor
                    senderId        : userId,
                    content         : safeContent,
                    type,
                    ...
                    is_deleted      : false,
                    isDeleted       : false,
                    created_at      : now,              // HTML msg.created_at bekliyor
                    createdAt       : now,
                    tempId          : tempId || null,
                    read_by         : [userId],
                    readBy          : [userId],
                    sender: {
                        id           : sender.id,
                        name         : sender.name,
                        display_name : sender.name,    // HTML sender.display_name bekliyor
                        username     : sender.username,
                        avatar_url   : senderPic,      // HTML sender.avatar_url bekliyor
...
// =============================================================================
// 💬 CHATSEE — Tamamen Bağımsız Mesajlaşma Platformu
//    Tüm route'lar /api/chatsee/ ile başlar — AgroLink route'larıyla çakışmaz
//    Tablolar: chatsee_conversations, chatsee_members, chatsee_messages, chatsee_reads
//    Frontend: public/chatsee/index.html → /chatsee URL'i üzerinden erişilir
// =============================================================================

// ── Yardımcı: ChatSee avatar URL ─────────────────────────────────────────────
function csAvatar(pic, seed) {
    if (pic) return absoluteUrl(pic);
    return `https://api.dicebear.com/7.x/thumbs/svg?seed=${encodeURIComponent(seed||'u')}&backgroundColor=0a1628`;
}

// ── Statik: GET /chatsee ─────────────────────────────────────────────────────
app.get('/chatsee', (req, res) => {
    const p = path.join(__dirname, 'public', 'chatsee', 'index.html');
    if (fssync.existsSync(p)) { res.setHeader('Content-Type','text/html; charset=utf-8'); res.sendFile(p); }
    else res.status(404).send('ChatSee kurulmamış: public/chatsee/index.html ekleyin');
});
app.get('/chatsee/*', (req, res) => {
    const p = path.join(__dirname, 'public', 'chatsee', 'index.html');
    if (fssync.existsSync(p)) { res.setHeader('Content-Type','text/html; charset=utf-8'); res.sendFile(p); }
...
app.post('/api/chatsee/auth/register', async (req, res) => {
    try {
        const { name, display_name, username, email, password } = req.body;
        ...
        if (!finalName || !cleanUsername || !cleanEmail || !pwd) {
            const m = [];
            if (!finalName)     m.push('İsim');
            if (!cleanUsername) m.push('Kullanıcı adı');
            if (!cleanEmail)    m.push('E-posta');
            if (!pwd)           m.push('Şifre');
            return res.status(400).json({ error: `Şu alanlar zorunlu: ${m.join(', ')}` });
        }
        if (pwd.length < 8)          return res.status(400).json({ error: 'Şifre en az 8 karakter olmalı' });
        if (cleanUsername.length < 3) return res.status(400).json({ error: 'Kullanıcı adı en az 3 karakter' });

        const exU = await dbGet('SELECT id FROM users WHERE username=$1', [cleanUsername]);
        if (exU) return res.status(409).json({ error: 'Bu kullanıcı adı zaten alınmış' });
        const exE = await dbGet('SELECT id FROM users WHERE email=$1', [cleanEmail]);
        if (exE) return res.status(409).json({ error: 'Bu e-posta zaten kayıtlı' });

        const hash   = await bcrypt.hash(pwd, BCRYPT_ROUNDS);
        const userId = uuidv4();

        await dbRun(
            `INSERT INTO users (id,name,username,email,password,"userType","registrationIp",
              "isOnline","emailVerified","twoFactorEnabled","createdAt","updatedAt")
             VALUES ($1,$2,$3,$4,$5,'normal_kullanici',$6,FALSE,TRUE,FALSE,NOW(),NOW())`,
            [userId, finalName, cleanUsername, cleanEmail, hash, req.ip||'']
        );

        const tokens  = generateTokens({ id:userId, username:cleanUsername, email:cleanEmail, role:'user' });
        setAuthCookies(res, req, tokens);
        const pic = csAvatar(null, cleanUsername);

        res.status(201).json({
            success:true, token:tokens.accessToken, refreshToken:tokens.refreshToken,
            user:{ id:userId, name:finalName, display_name:finalName, username:cleanUsername,
                   email:cleanEmail, avatar_url:pic, profilePic:pic, role:'user', isVerified:false }
        });
    } catch (e) {
        console.error('[ChatSee] register:', e.message);
        if (e.code==='23505') return res.status(409).json({ error: 'Kullanıcı adı veya e-posta zaten kayıtlı' });
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.post('/api/chatsee/auth/login', async (req, res) => {
    try {
        ...
        const user = await dbGet(
            `SELECT id,username,name,email,password,role,plan,"profilePic","isVerified",
                    "isActive","isBanned","hasFarmerBadge"
             FROM users WHERE (email=$1 OR username=$1) AND "isActive"=TRUE`,
            [loginId]
        );
        ...
        await dbRun(`UPDATE users SET "isOnline"=TRUE,"lastLogin"=NOW(),"updatedAt"=NOW() WHERE id=$1`, [user.id]);

        const tokens = generateTokens(user);
        setAuthCookies(res, req, tokens);
        const pic = csAvatar(user.profilePic, user.username);

        res.json({
            success:true, token:tokens.accessToken, refreshToken:tokens.refreshToken,
            user:{ id:user.id, name:user.name, display_name:user.name, username:user.username,
                   email:user.email, avatar_url:pic, profilePic:pic,
                   isVerified:user.isVerified, hasFarmerBadge:user.hasFarmerBadge, role:user.role }
        });
    } catch (e) {
        console.error('[ChatSee] login:', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// ── GET /api/chatsee/auth/me ──────────────────────────────────────────────────
app.get('/api/chatsee/auth/me', authenticateToken, async (req, res) => {
    try {
        const user = await dbGet(
            `SELECT id,name,username,email,"profilePic","isVerified","hasFarmerBadge",role,plan,"isOnline"
             FROM users WHERE id=$1 AND "isActive"=TRUE`,
            [req.user.id]
        );
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
        const pic = csAvatar(user.profilePic, user.username);
        res.json({ id:user.id, name:user.name, display_name:user.name, username:user.username,
                   email:user.email, avatar_url:pic, profilePic:pic,
                   isVerified:user.isVerified, hasFarmerBadge:user.hasFarmerBadge,
                   role:user.role, plan:user.plan, isOnline:user.isOnline });
    } catch (e) { res.status(500).json({ error: 'Sunucu hatası' }); }
});

// ── POST /api/chatsee/auth/logout ─────────────────────────────────────────────
app.post('/api/chatsee/auth/logout', authenticateToken, async (req, res) => {
    await dbRun(`UPDATE users SET "isOnline"=FALSE,"lastSeen"=NOW() WHERE id=$1`, [req.user.id]).catch(()=>{});
    res.clearCookie('access_token'); res.clearCookie('refresh_token');
    res.json({ success:true });
});

// ── GET /api/chatsee/token — Socket.IO için JWT ───────────────────────────────
app.get('/api/chatsee/token', authenticateToken, (req, res) => {
    const t = generateTokens(req.user);
    res.json({ token: t.accessToken });
});

// ── GET /api/chatsee/users/search?q= ─────────────────────────────────────────
app.get('/api/chatsee/users/search', authenticateToken, async (req, res) => {
    const q = (req.query.q||'').trim().toLowerCase();
    if (q.length < 2) return res.json([]);
    try {
        const { rows } = await pool.query(`
            SELECT u.id, u.name, u.username, u."profilePic", u."isOnline", u."isVerified"
            FROM users u
            WHERE (LOWER(u.username) LIKE $1 OR LOWER(u.name) LIKE $1)
              AND u.id!=$2 AND u."isActive"=TRUE AND u."isBanned"=FALSE
              AND NOT EXISTS(SELECT 1 FROM blocks b WHERE
                    (b."blockerId"=$2 AND b."blockedId"=u.id) OR
                    (b."blockerId"=u.id AND b."blockedId"=$2))
            ORDER BY u."isOnline" DESC, u.username LIMIT 20
        `, [`%${q}%`, req.user.id]);
        res.json(rows.map(r => ({
            id:r.id, name:r.name, display_name:r.name, username:r.username,
            avatar_url:csAvatar(r.profilePic, r.username),
            status:r.isOnline?'online':'offline', isOnline:!!r.isOnline, isVerified:r.isVerified
        })));
    } catch (e) { res.status(500).json({ error: 'Sunucu hatası' }); }
});

// ── GET /api/chatsee/conversations ───────────────────────────────────────────
app.get('/api/chatsee/conversations', authenticateToken, async (req, res) => {
    try {
        const uid = req.user.id;
        const { rows } = await pool.query(`
            SELECT c.id, c.type, c.name AS group_name, c.avatar_url, c."updatedAt",
                   lm.content AS last_message, lm."createdAt" AS last_message_at,
                   ...
                   (SELECT COUNT(*)::int FROM chatsee_messages m2
                    ...
                    WHERE m2.conversation_id=c.id AND m2.sender_id!=$1
                      AND m2.is_deleted=FALSE AND r2.message_id IS NULL) AS unread_count,
                   ou.id AS other_id, ou.name AS other_name, ou.username AS other_username,
                   ou."profilePic" AS other_pic, ou."isOnline" AS other_online, ou."lastSeen" AS other_last_seen
            ...
            LEFT JOIN LATERAL(
                ...
                FROM chatsee_members cm2 JOIN users u ON u.id=cm2.user_id
        ...
        res.json(rows.map(r => {
            ...
            const dispName  = isDirect ? (r.other_name  || r.other_username || 'Kullanıcı') : (r.group_name  || 'Grup');
            const dispPic   = isDirect ? av(r.other_pic, r.other_username) : av(r.avatar_url, r.group_name);
            const unread    = parseInt(r.unread_count || 0);
            return {
                id               : r.id,
                type             : r.type,
                // HTML renderConvList eski alan adlarını kullanıyor — ikisini de gönder
                name             : dispName,
                other_display_name: dispName,
                username         : isDirect ? r.other_username : null,
                other_username   : isDirect ? r.other_username : null,
                avatar_url       : dispPic,
                other_avatar_url : dispPic,
                isOnline         : isDirect ? !!r.other_online : false,
                other_status     : (isDirect && r.other_online) ? 'online' : 'offline',
        ...
        console.error('[ChatSee] GET /conversations:', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// ── POST /api/chatsee/conversations/direct ────────────────────────────────────
app.post('/api/chatsee/conversations/direct', authenticateToken, async (req, res) => {
    try {
        ...
        `, [myId, target]);
        if (ex.rows.length) return res.json({ conversation_id:ex.rows[0].id, created:false });

        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const cid = uuidv4();
            await client.query(`INSERT INTO chatsee_conversations(id,type,created_by) VALUES($1,'direct',$2)`, [cid,myId]);
            await client.query(`INSERT INTO chatsee_members(conversation_id,user_id) VALUES($1,$2),($1,$3)`, [cid,myId,target]);
            await client.query('COMMIT');
            res.status(201).json({ conversation_id:cid, created:true });
        } catch(err){ await client.query('ROLLBACK'); throw err; }
        finally { client.release(); }
    } catch (e) {
        console.error('[ChatSee] POST /direct:', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// ── POST /api/chatsee/conversations/group ─────────────────────────────────────
app.post('/api/chatsee/conversations/group', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await client.query('BEGIN');
            const cid = uuidv4();
            await client.query(`INSERT INTO chatsee_conversations(id,type,name,created_by) VALUES($1,'group',$2,$3)`,[cid,name,myId]);
            for (const uid of allIds)
                await client.query(`INSERT INTO chatsee_members(conversation_id,user_id,is_admin) VALUES($1,$2,$3)`,[cid,uid,uid===myId]);
            await client.query('COMMIT');
...
// ── GET /api/chatsee/conversations/:id/messages ───────────────────────────────
app.get('/api/chatsee/conversations/:id/messages', authenticateToken, async (req, res) => {
    try {
        ...
        const mem = await pool.query(`SELECT 1 FROM chatsee_members WHERE conversation_id=$1 AND user_id=$2`,[convId,myId]);
        ...
        const unread = rows.filter(r=>r.sender_id!==myId).map(r=>r.id);
        if (unread.length) {
            pool.query(`INSERT INTO chatsee_reads(message_id,user_id,"readAt") SELECT unnest($1::uuid[]),$2,NOW() ON CONFLICT DO NOTHING`,[unread,myId]).catch(()=>{});
            pool.query(`UPDATE chatsee_members SET last_read_at=NOW() WHERE conversation_id=$1 AND user_id=$2`,[convId,myId]).catch(()=>{});
        }

        res.json(rows.reverse().map(r => ({
            id:r.id, conversation_id:r.conversation_id, sender_id:r.sender_id,
            content:r.content, type:r.type, reply_to_id:r.reply_to_id,
            is_deleted:r.is_deleted, created_at:r.createdAt, read_by:r.read_by||[],
            sender:{
                id:r.sender_id, name:r.sender_name, display_name:r.sender_name,
                username:r.sender_username, avatar_url:csAvatar(r.sender_pic,r.sender_username),
                isVerified:r.sender_verified
            }
        })));
    } catch(e){
        console.error('[ChatSee] GET /messages:', e.message);
        res.status(500).json({ error:'Sunucu hatası' });
    }
});

// ── DELETE /api/chatsee/messages/:id ─────────────────────────────────────────
app.delete('/api/chatsee/messages/:id', authenticateToken, async (req, res) => {
    try {
        const { rows } = await pool.query(
            `UPDATE chatsee_messages SET is_deleted=TRUE,content='Bu mesaj silindi.',type='deleted'
             WHERE id=$1 AND sender_id=$2 RETURNING id,conversation_id`,
            [req.params.id, req.user.id]
        );
        if (!rows.length) return res.status(403).json({ error: 'İzin yok' });
        if (io) io.to(`cs:${rows[0].conversation_id}`).emit('chatsee:deleted',{messageId:rows[0].id,conversationId:rows[0].conversation_id});
        res.json({ success:true });
    } catch(e){ res.status(500).json({ error:'Sunucu hatası' }); }
});

// ── GET /api/chatsee/conversations/:id/members ────────────────────────────────
app.get('/api/chatsee/conversations/:id/members', authenticateToken, async (req, res) => {
    try {
        const mem = await pool.query(`SELECT 1 FROM chatsee_members WHERE conversation_id=$1 AND user_id=$2`,[req.params.id,req.user.id]);
        if (!mem.rows.length) return res.status(403).json({ error:'Erişim yok' });
        const { rows } = await pool.query(`
            SELECT u.id,u.name,u.username,u."profilePic",u."isOnline",u."lastSeen",m.is_admin
            FROM chatsee_members m JOIN users u ON u.id=m.user_id
            WHERE m.conversation_id=$1 ORDER BY m.is_admin DESC,u.name
        `,[req.params.id]);
        res.json(rows.map(r=>({...r, avatar_url:csAvatar(r.profilePic,r.username)})));
    } catch(e){ res.status(500).json({ error:'Sunucu hatası' }); }
});
...
app.get('*', (req, res, next) => {
    ...
    if (fss.existsSync(htmlPath)) {
        ...
        res.status(404).json({ error: 'Sayfa bulunamıdı' });
    }
});
